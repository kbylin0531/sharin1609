...
$web_root = str_replace(P($_SERVER['SCRIPT_NAME']),'',P(dirname(dirname(__FILE__))).'/'.ENTRY_FILE).'/';
if (substr($web_root,-10) == ENTRY_FILE.'/') {//解决部分主机不兼容问题
...
define('BASIC_PATH',    SR_PATH_APP.'/');//P(dirname(dirname(__FILE__))).'/'
...
include(BASIC_PATH.'Common/config/setting.php');
include(BASIC_PATH.'Common/config/version.php');

//数据地址定义。
$config['pic_thumb']	= DATA_PATH.'thumb/';		// 缩略图生成存放地址
$config['cache_dir']	= DATA_PATH.'cache/';		// 缓存文件地址
